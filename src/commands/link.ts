...
export function init(store: Store<StoreData>, sweep: Sweep): void {
  command.register(LinkConfig.name, LinkConfig.meta, CommandPermissionLevel.Admin)
    .overload((p, e, o) => {
      // Make Request To Get Store Info To Ensure Correct Token

      new RestClient(p[LinkConfig.params.secret])
        .api
        .information
        ...
        .then((r) => {
          ...
          store.set('tebex_secret', p[LinkConfig.params.secret])
          store.save()

          sweep.start()
        })
        .catch((err: AxiosError) => {
          console.error(`[plugin:Tebexio]`, `Failed to change token`.grey, '::'.white, `(${err.response?.status}) - ${err.response?.statusText}`.red, '-'.white, `${err.response?.data.error_message ?? 'Unknown Reason'}`.grey)
        })
    }, {
