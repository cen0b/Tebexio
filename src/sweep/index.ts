...
export class Sweep extends EventEmitter {
    ...
    protected async __sweepInterval(): Promise<void> {
        // Handle online players
        for (const [xuid, player] of this.onlinePlayers) {
            if (!this.duePlayers.has(xuid)) {
                continue;
            }
            const due = this.duePlayers.get(xuid)!;
            try {
                const data = await this._getCommands(due.id);
                const completed: number[] = [];
                for (const command of data.commands) {
                    const final = command.command.replace(/{id}/gi, xuid).replace(/{username}/gi, player.getName());
                    const emptySlots = player
                        .getInventory()
                        .getSlots()
                        .toArray()
                        .map(i => i.getName())
                        .filter(name => name === "minecraft:air").length;
                    if (command.conditions.slots && emptySlots < command.conditions.slots) {
                        player.sendMessage(`§cCannot claim reward, inventory needs ${command.conditions.slots} empty slots! Trying again in 10 seconds...`);
                        continue;
                    }
                    setTimeout(() => {
                        bedrockServer.executeCommand(final);
                        console.log(`[plugin:Tebexio]`, `Doing ${player.getName().cyan}${`'s transaction ${command.id}`.grey}`.grey, "::".white, `${final}`.grey);
                ...
                if (completed.length) {
                    const complete = async (): Promise<void> => {
                        try {
                            await this._markAsComplete(completed);
                            console.log(
                                `[plugin:Tebexio]`,
                ...
                .then(data => {
                    ...
                    const complete = (): void => {
                        this._markAsComplete(completed)
                            .then(() => {
                                console.log(`[plugin:Tebexio]`, `Completing offline transactions`.grey, "::".white, `[${completed.join(", ")}]`.grey);
                            })
                            .catch((err: AxiosError) => {
                                console.error(
                                    `[plugin:Tebexio]`,
                    ...
                    for (const command of data.commands) {
                        const final = command.command.replace(/{id}/gi, String(command.player.id)).replace(/{username}/gi, command.player.name);
    ...
    protected async _getCommands(transactionId: number): Promise<TebexDueOnlineCommands> {
        try {
            const { data } = await this.client.api.queue["online-commands"][transactionId].get<TebexDueOnlineCommands>();
            return data;
        } catch (e) {
            throw e;
        }
    ...
    protected async _getOfflineCommands(): Promise<TebexDueOfflineCommands> {
        try {
            const { data } = await this.client.api.queue["offline-commands"].get<TebexDueOfflineCommands>();
            return data;
        } catch (e) {
            throw e;
        }
    ...
    protected async _markAsComplete(ids: number[]): Promise<void> {
        try {
            await this.client.api.queue.delete({
                data: {
                    ids,
                },
            });
